<template>
	<view class="wrap">
		...
		<!-- 提交按钮 -->
        <view class="submitNow" v-if="artList.length > 0" @tap="submitNow">重新发布</view>				
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			uploadImg : function(){
				...
				if(this.needUploadImg.length < 1 || this.uploadIndex >=  this.needUploadImg.length){
					...
					uni.request({
						url: this.apiServer + this.editArticle,
						method: 'PUT',
		...
		onLoad:function(options){
			...
			// 动态加载文章分类
			uni.request({
				url: this.apiServer + this.artInfo,
				method: 'GET',
				data:{
					artId:artId,
				},
				// 获取文章内容
				success: res => {
					console.log('文章内容'+JSON.stringify(res.data.result));
					var art = res.data.result;
					// 文章内容	
					var artContent = art.artContent;
					artContent = JSON.parse(artContent);
					_self.artList = artContent;
					// 获取标题
					this.title = art.artTitle;
					// 此方法获取文章分类
					uni.request({
